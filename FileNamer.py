...
# to get the outer right side border that was cut off for some weird reason
for i in range(9):
    target = colour_goals[i][final_width - 1]
    lowest = []

    for element in vals:
        newmin = spatial.distance.pdist([element, target])
        lowest.append(newmin[0])
        if len(lowest) == len(vals):
            print(min(lowest))
            smallest_pos = (lowest.index(min(lowest)))
            ...
            order[i][final_width - 1] = (keys[smallest_pos])

# to get the outer bottom side that was cut off for some weird reason
for i in range(20):
    target = colour_goals[final_height - 1][i]
    lowest = []
    for element in vals:
        newmin = spatial.distance.pdist([element, target])
        lowest.append(newmin[0])
        if len(lowest) == len(vals):
            ...
            order[final_height - 1][i] = (keys[smallest_pos])

# finally sticking them in a list and changing the names
final = []
left = []
count = 0
...
# for element in dom_colour.keys():
...
for i in range(len(final)):
    ...
    name = '/Users/rijulmahajan/Desktop/Edits/'
    # if os.path.exists(name + str(i) + ".jpg"):
    #     # print(i)
    #     pass
    # else:
    os.rename(name + str(final[i]), name + str(i+1) + ".jpg")
